...
export function loadMapFromLocalStorage(ls_key: string): MapState | null {
  try {
    ...
    if (
      isPlainObject(parsed.boardPieces) &&
      Object.keys(parsed.boardPieces).length &&
      typeof Object.keys(parsed.boardPieces)[0] === 'string'
    ) {
      const migrated1 = inflateBoardPiecesFromIds(
        normalizeBoardPieces(parsed.boardPieces),
      )
      return { hexMap: parsed.hexMap, boardPieces: migrated1 }
    }
    // handle legacy format where map stored boardPieces as string[]
    if (
      Array.isArray(parsed.boardPieces) &&
      parsed.boardPieces.length &&
      typeof parsed.boardPieces[0] === 'string'
    ) {
      const migrated2 = inflateBoardPiecesFromIds(
        parsed.boardPieces as string[],
      )
    ...
    // already new shape
    const existingMap = {
      hexMap: parsed.hexMap,
      boardPieces: parsed.boardPieces,
    }
    return existingMap
  } catch (e) {
