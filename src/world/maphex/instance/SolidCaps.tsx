...
const SolidCaps = ({
  ...
  const { nodes: roadNodes, materials: roadMaterials } = useGLTF(
    '/road-cap-textured.glb',
    // biome-ignore lint/suspicious/noExplicitAny: <mesh names from Blender>
  ) as any
  ...
  return (
    <>
      ...
      {roadHexes.length > 0 &&
        renderCaps(roadHexes, roadHighQualityGeometry, roadRef)}
    </>
  )
}
...
function SolidCapInstance({
  ...
  const color = isHighQualityRender
    ? '#EBEBEB'
    : terrainCapColors[boardHex.terrain]
