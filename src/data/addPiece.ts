...
export function addPieceToBoardHexes({
  ...
  const isStartZonePiece = pieceData.terrain === HexTerrain.startZone
  // Validate
  ...
  const isVerticalClearanceForPiece = newHexIds.every((_, i) => {
    const clearanceHexIds = Array(
      verticalObstructionTemplates?.[pieceData.id]?.[i] ?? pieceData.height,
    )
      .fill(0)
      .map((_, j) => {
  ...
  const isObstaclePieceSupported =
    isSolidUnderAll ||
    ((pieceData.id === Pieces.laurWallSquarePillar ||
      pieceData.id === Pieces.laurWallTrianglePillar ||
      isGlyphPiece ||
      isStartZonePiece) &&
      isLandUnderAll) || // Laur wall pillars, and glyphs, can be placed on fluid tiles, per Renegade
    (isBridgingObstaclePieceID(pieceData.id) && isSolidUnderAtLeastOne) || // some multi-hex fluid-tile based obstacles (glaciers-4/6, hive) can bridge over gaps
        ...
        Array(pieceData.height)
          ...
          .forEach((_, j) => {
            ...
            newBoardHexes[clearanceID] = {
              ...
              isObstacleAuxiliary: false,
              obstacleHeight: pieceData.height, // probably unused
  ...
  // RUINS
  if (pieceData.terrain === HexTerrain.ruin) {
    const isSolidUnderAllSupportHexes = underHexIds.every((_, i) => {
      ...
      const isRequiredToSupportThisOne =
        verticalSupportTemplates?.[pieceData.id]?.[i]
    ...
    if (isPlacingRuin) {
      newHexIds.forEach((newHexID, i) => {
        ...
        Array(verticalObstructionTemplates[pieceData.id][i])
          .fill(0)
          .forEach((_, j) => {
            if (j === 0) {
  ...
  // CASTLE BASE
  if (pieceData.id.includes(PiecePrefixes.castleBase)) {
  ...
  if (isCastleArchPiece) {
    ...
    if (isPlacingCastleArch) {
      newHexIds.forEach((newHexID, i) => {
        const hexUnderneath = newBoardHexes?.[underHexIds[i]]
        const obstacleHeight = pieceData.height
  ...
  if (isPlacingObstacle) {
    newHexIds.forEach((newHexID, i) => {
      ...
      if (verticalObstructionTemplates[pieceData.id]) {
        try {
      ...
      try {
        newHexIds.forEach((newHexID, iForEach) => {
          ...
          newBoardHexes[newHexID] = {
            ...
            isObstacleOrigin: iForEach === 0, // mark subterrain origin hex
            isObstacleAuxiliary: iForEach !== 0, // mark non-origin hex
            interlockType: interlockTemplates[pieceData.template][iForEach],
            interlockRotation:
              interlockRotationTemplates[pieceData.template][iForEach],
          }
        })
      } catch (error) {
        console.log("🚀 ~ addPieceToBoardHexes ~ error:", error)
