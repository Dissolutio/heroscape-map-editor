import type { Dictionary } from 'lodash'
import { HexTerrain, type Piece, PiecePrefixes, Pieces } from '../types'
import {
  powerGlyphs,
  treasureGlyphs,
  marvelGlyphs,
  c3vGlyphs,
  c3vPlaytestGlyphs,
  customGlyphs,
} from './glyphs'

export const getPieceByTerrainAndSize = (terrain: string, size: number) => {
  const piece = piecesSoFar[`${terrain}${size}`]
  return piece
}
export const piecesSoFar: Dictionary<Piece> = {
  // UNINVENTORIED
  [Pieces.startZone1]: {
    id: Pieces.startZone1,
    title: 'Start Zone: Player 1',
    terrain: HexTerrain.startZone,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.startZone2]: {
    id: Pieces.startZone2,
    title: 'Start Zone: Player 2',
    terrain: HexTerrain.startZone,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.startZone3]: {
    id: Pieces.startZone3,
    title: 'Start Zone: Player 3',
    terrain: HexTerrain.startZone,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.startZone4]: {
    id: Pieces.startZone4,
    title: 'Start Zone: Player 4',
    terrain: HexTerrain.startZone,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.startZone5]: {
    id: Pieces.startZone5,
    title: 'Start Zone: Player 5',
    terrain: HexTerrain.startZone,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.startZone6]: {
    id: Pieces.startZone6,
    title: 'Start Zone: Player 6',
    terrain: HexTerrain.startZone,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.startZone7]: {
    id: Pieces.startZone7,
    title: 'Start Zone: Player 7',
    terrain: HexTerrain.startZone,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.startZone8]: {
    id: Pieces.startZone8,
    title: 'Start Zone: Player 8',
    terrain: HexTerrain.startZone,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.objectiveMarkerType11]: {
    id: Pieces.objectiveMarkerType11,
    title: 'Objective Marker Type 1 (1-hex)',
    terrain: HexTerrain.objectiveMarker,
    landPrefix: PiecePrefixes.objectiveMarker1,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.objectiveMarkerType12]: {
    id: Pieces.objectiveMarkerType12,
    title: 'Objective Marker Type 1 (2-hex)',
    terrain: HexTerrain.objectiveMarker,
    landPrefix: PiecePrefixes.objectiveMarker1,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 2,
    template: '2',
    height: 0,
  },
  [Pieces.objectiveMarkerType21]: {
    id: Pieces.objectiveMarkerType21,
    title: 'Objective Marker Type 2 (1-hex)',
    terrain: HexTerrain.objectiveMarker,
    landPrefix: PiecePrefixes.objectiveMarker2,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.objectiveMarkerType22]: {
    id: Pieces.objectiveMarkerType22,
    title: 'Objective Marker Type 2 (2-hex)',
    terrain: HexTerrain.objectiveMarker,
    landPrefix: PiecePrefixes.objectiveMarker2,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 2,
    template: '2',
    height: 0,
  },
  [Pieces.glyphPower]: {
    id: Pieces.glyphPower,
    title: 'Power Glyph',
    terrain: HexTerrain.glyphPower,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
    glyphLetter: '?',
  },
  [Pieces.glyphTreasure]: {
    id: Pieces.glyphTreasure,
    title: 'Treasure Glyph',
    terrain: HexTerrain.glyphTreasure,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    isOverlayPiece: true,
    size: 1,
    template: '1',
    height: 0,
    glyphLetter: '?',
  },

  // INVENTORIED

  // LAND
  [Pieces.grass1]: {
    id: Pieces.grass1,
    title: '1-hex Grass',
    terrain: HexTerrain.grass,
    landPrefix: PiecePrefixes.grass,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.grass2]: {
    id: Pieces.grass2,
    title: '2-hex Grass',
    terrain: HexTerrain.grass,
    landPrefix: PiecePrefixes.grass,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.grass3]: {
    id: Pieces.grass3,
    title: '3-hex Grass',
    terrain: HexTerrain.grass,
    landPrefix: PiecePrefixes.grass,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.grass7]: {
    id: Pieces.grass7,
    title: '7-hex Grass',
    terrain: HexTerrain.grass,
    landPrefix: PiecePrefixes.grass,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.grass24]: {
    id: Pieces.grass24,
    title: '24-hex Grass',
    terrain: HexTerrain.grass,
    landPrefix: PiecePrefixes.grass,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.rock1]: {
    id: Pieces.rock1,
    title: '1-hex Rock',
    terrain: HexTerrain.rock,
    landPrefix: PiecePrefixes.rock,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.rock2]: {
    id: Pieces.rock2,
    title: '2-hex Rock',
    terrain: HexTerrain.rock,
    landPrefix: PiecePrefixes.rock,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.rock3]: {
    id: Pieces.rock3,
    title: '3-hex Rock',
    terrain: HexTerrain.rock,
    landPrefix: PiecePrefixes.rock,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.rock7]: {
    id: Pieces.rock7,
    title: '7-hex Rock',
    terrain: HexTerrain.rock,
    landPrefix: PiecePrefixes.rock,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.rock24]: {
    id: Pieces.rock24,
    title: '24-hex Rock',
    terrain: HexTerrain.rock,
    landPrefix: PiecePrefixes.rock,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.sand1]: {
    id: Pieces.sand1,
    title: '1-hex Sand',
    terrain: HexTerrain.sand,
    landPrefix: PiecePrefixes.sand,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.sand2]: {
    id: Pieces.sand2,
    title: '2-hex Sand',
    terrain: HexTerrain.sand,
    landPrefix: PiecePrefixes.sand,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.sand3]: {
    id: Pieces.sand3,
    title: '3-hex Sand',
    terrain: HexTerrain.sand,
    landPrefix: PiecePrefixes.sand,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.sand7]: {
    id: Pieces.sand7,
    title: '7-hex Sand',
    terrain: HexTerrain.sand,
    landPrefix: PiecePrefixes.sand,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.sand24]: {
    id: Pieces.sand24,
    title: '24-hex Sand',
    terrain: HexTerrain.sand,
    landPrefix: PiecePrefixes.sand,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.dungeon1]: {
    id: Pieces.dungeon1,
    title: '1-hex Dungeon',
    terrain: HexTerrain.dungeon,
    landPrefix: PiecePrefixes.dungeon,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.dungeon2]: {
    id: Pieces.dungeon2,
    title: '2-hex Dungeon',
    terrain: HexTerrain.dungeon,
    landPrefix: PiecePrefixes.dungeon,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.dungeon3]: {
    id: Pieces.dungeon3,
    title: '3-hex Dungeon',
    terrain: HexTerrain.dungeon,
    landPrefix: PiecePrefixes.dungeon,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.dungeon7]: {
    id: Pieces.dungeon7,
    title: '7-hex Dungeon',
    terrain: HexTerrain.dungeon,
    landPrefix: PiecePrefixes.dungeon,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.dungeon24]: {
    id: Pieces.dungeon24,
    title: '24-hex Dungeon',
    terrain: HexTerrain.dungeon,
    landPrefix: PiecePrefixes.dungeon,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.swamp1]: {
    id: Pieces.swamp1,
    title: '1-hex Swamp',
    terrain: HexTerrain.swamp,
    landPrefix: PiecePrefixes.swamp,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.swamp2]: {
    id: Pieces.swamp2,
    title: '2-hex Swamp',
    terrain: HexTerrain.swamp,
    landPrefix: PiecePrefixes.swamp,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.swamp3]: {
    id: Pieces.swamp3,
    title: '3-hex Swamp',
    terrain: HexTerrain.swamp,
    landPrefix: PiecePrefixes.swamp,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.swamp7]: {
    id: Pieces.swamp7,
    title: '7-hex Swamp',
    terrain: HexTerrain.swamp,
    landPrefix: PiecePrefixes.swamp,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.swamp24]: {
    id: Pieces.swamp24,
    title: '24-hex Swamp',
    terrain: HexTerrain.swamp,
    landPrefix: PiecePrefixes.swamp,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.snow1]: {
    id: Pieces.snow1,
    title: '1-hex Snow',
    terrain: HexTerrain.snow,
    landPrefix: PiecePrefixes.snow,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.snow2]: {
    id: Pieces.snow2,
    title: '2-hex Snow',
    terrain: HexTerrain.snow,
    landPrefix: PiecePrefixes.snow,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.snow3]: {
    id: Pieces.snow3,
    title: '3-hex Snow',
    terrain: HexTerrain.snow,
    landPrefix: PiecePrefixes.snow,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.snow7]: {
    id: Pieces.snow7,
    title: '7-hex Snow',
    terrain: HexTerrain.snow,
    landPrefix: PiecePrefixes.snow,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.snow24]: {
    id: Pieces.snow24,
    title: '24-hex Snow',
    terrain: HexTerrain.snow,
    landPrefix: PiecePrefixes.snow,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.lavaField1]: {
    id: Pieces.lavaField1,
    title: '1-hex Lava Field',
    terrain: HexTerrain.lavaField,
    landPrefix: PiecePrefixes.lavaField,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.lavaField2]: {
    id: Pieces.lavaField2,
    title: '2-hex Lava Field',
    terrain: HexTerrain.lavaField,
    landPrefix: PiecePrefixes.lavaField,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.lavaField3]: {
    id: Pieces.lavaField3,
    title: '3-hex Lava Field',
    terrain: HexTerrain.lavaField,
    landPrefix: PiecePrefixes.lavaField,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.lavaField7]: {
    id: Pieces.lavaField7,
    title: '7-hex Lava Field',
    terrain: HexTerrain.lavaField,
    landPrefix: PiecePrefixes.lavaField,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.lavaField24]: {
    id: Pieces.lavaField24,
    title: '24-hex Lava Field',
    terrain: HexTerrain.lavaField,
    landPrefix: PiecePrefixes.lavaField,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.asphalt1]: {
    id: Pieces.asphalt1,
    title: '1-hex Asphalt',
    terrain: HexTerrain.asphalt,
    landPrefix: PiecePrefixes.asphalt,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.asphalt2]: {
    id: Pieces.asphalt2,
    title: '2-hex Asphalt',
    terrain: HexTerrain.asphalt,
    landPrefix: PiecePrefixes.asphalt,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.asphalt3]: {
    id: Pieces.asphalt3,
    title: '3-hex Asphalt',
    terrain: HexTerrain.asphalt,
    landPrefix: PiecePrefixes.asphalt,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.asphalt7]: {
    id: Pieces.asphalt7,
    title: '7-hex Asphalt',
    terrain: HexTerrain.asphalt,
    landPrefix: PiecePrefixes.asphalt,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.asphalt24]: {
    id: Pieces.asphalt24,
    title: '24-hex Asphalt',
    terrain: HexTerrain.asphalt,
    landPrefix: PiecePrefixes.asphalt,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.concrete1]: {
    id: Pieces.concrete1,
    title: '1-hex Concrete',
    terrain: HexTerrain.concrete,
    landPrefix: PiecePrefixes.concrete,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.concrete2]: {
    id: Pieces.concrete2,
    title: '2-hex Concrete',
    terrain: HexTerrain.concrete,
    landPrefix: PiecePrefixes.concrete,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.concrete3]: {
    id: Pieces.concrete3,
    title: '3-hex Concrete',
    terrain: HexTerrain.concrete,
    landPrefix: PiecePrefixes.concrete,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.concrete6]: {
    id: Pieces.concrete6,
    title: '6-hex Concrete',
    terrain: HexTerrain.concrete,
    landPrefix: PiecePrefixes.concrete,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 6,
    template: Pieces.marvel,
    height: 1,
  },
  [Pieces.concrete7]: {
    id: Pieces.concrete7,
    title: '7-hex Concrete',
    terrain: HexTerrain.concrete,
    landPrefix: PiecePrefixes.concrete,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.concrete24]: {
    id: Pieces.concrete24,
    title: '24-hex Concrete',
    terrain: HexTerrain.concrete,
    landPrefix: PiecePrefixes.concrete,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.road1]: {
    id: Pieces.road1,
    title: '1-hex Road',
    terrain: HexTerrain.road,
    landPrefix: PiecePrefixes.road,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.road2]: {
    id: Pieces.road2,
    title: '2-hex Road',
    terrain: HexTerrain.road,
    landPrefix: PiecePrefixes.road,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.road5]: {
    id: Pieces.road5,
    title: '5-hex Road',
    terrain: HexTerrain.road,
    landPrefix: PiecePrefixes.road,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 5,
    template: '5',
    height: 1,
  },
  [Pieces.wallWalk1]: {
    id: Pieces.wallWalk1,
    title: '1-hex Wall Walk',
    terrain: HexTerrain.wallWalk,
    landPrefix: PiecePrefixes.wallWalk,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.wallWalk7]: {
    id: Pieces.wallWalk7,
    title: '7-hex Wall Walk',
    terrain: HexTerrain.wallWalk,
    landPrefix: PiecePrefixes.wallWalk,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: Pieces.wallWalk7,
    height: 1,
  },
  [Pieces.wallWalk9]: {
    id: Pieces.wallWalk9,
    title: '9-hex Wall Walk',
    terrain: HexTerrain.wallWalk,
    landPrefix: PiecePrefixes.wallWalk,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 9,
    template: Pieces.wallWalk9,
    height: 1,
  },
  [Pieces.toxic1]: {
    id: Pieces.toxic1,
    title: '1-hex Toxic',
    terrain: HexTerrain.toxic,
    landPrefix: PiecePrefixes.toxic,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.ancientTerrain1]: {
    id: Pieces.ancientTerrain1,
    title: '1-hex Ancient Terrain',
    terrain: HexTerrain.ancientTerrain,
    landPrefix: PiecePrefixes.ancientTerrain,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.ancientTerrain2]: {
    id: Pieces.ancientTerrain2,
    title: '2-hex Ancient Terrain',
    terrain: HexTerrain.ancientTerrain,
    landPrefix: PiecePrefixes.ancientTerrain,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  [Pieces.ancientTerrain3]: {
    id: Pieces.ancientTerrain3,
    title: '3-hex Ancient Terrain',
    terrain: HexTerrain.ancientTerrain,
    landPrefix: PiecePrefixes.ancientTerrain,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 1,
  },
  [Pieces.ancientTerrain7]: {
    id: Pieces.ancientTerrain7,
    title: '7-hex Ancient Terrain',
    terrain: HexTerrain.ancientTerrain,
    landPrefix: PiecePrefixes.ancientTerrain,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 7,
    template: '7',
    height: 1,
  },
  [Pieces.ancientTerrain24]: {
    id: Pieces.ancientTerrain24,
    title: '24-hex Ancient Terrain',
    terrain: HexTerrain.ancientTerrain,
    landPrefix: PiecePrefixes.ancientTerrain,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 24,
    template: '24',
    height: 1,
  },
  [Pieces.wood2]: {
    id: Pieces.wood2,
    title: '2-hex Wood',
    terrain: HexTerrain.wood,
    landPrefix: PiecePrefixes.wood,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 2,
    template: '2',
    height: 1,
  },
  /* FLUID / HALF-HEIGHT PIECES */
  [Pieces.wellspringWater1]: {
    id: Pieces.wellspringWater1,
    title: '1-hex WellspringWater',
    terrain: HexTerrain.wellspringWater,
    landPrefix: PiecePrefixes.wellspringWater,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.toxicWater1]: {
    id: Pieces.toxicWater1,
    title: '1-hex Toxic Water',
    terrain: HexTerrain.toxicWater,
    landPrefix: PiecePrefixes.toxicWater,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.water1]: {
    id: Pieces.water1,
    title: '1-hex Water',
    terrain: HexTerrain.water,
    landPrefix: PiecePrefixes.water,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.water3]: {
    id: Pieces.water3,
    title: '3-hex Water',
    terrain: HexTerrain.water,
    landPrefix: PiecePrefixes.water,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 0,
  },
  [Pieces.lava1]: {
    id: Pieces.lava1,
    title: '1-hex Molten Lava',
    terrain: HexTerrain.lava,
    landPrefix: PiecePrefixes.lava,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.lava3]: {
    id: Pieces.lava3,
    title: '3-hex Molten Lava',
    terrain: HexTerrain.lava,
    landPrefix: PiecePrefixes.lava,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 0,
  },
  [Pieces.swampWater1]: {
    id: Pieces.swampWater1,
    title: '1-hex Swamp Water',
    terrain: HexTerrain.swampWater,
    landPrefix: PiecePrefixes.swampWater,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.swampWater3]: {
    id: Pieces.swampWater3,
    title: '3-hex Swamp Water',
    terrain: HexTerrain.swampWater,
    landPrefix: PiecePrefixes.swampWater,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 0,
  },
  [Pieces.swampWater6]: {
    id: Pieces.swampWater6,
    title: '6-hex Swamp Water',
    terrain: HexTerrain.swampWater,
    landPrefix: PiecePrefixes.swampWater,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 6,
    template: '6',
    height: 0,
  },
  [Pieces.ice1]: {
    id: Pieces.ice1,
    title: '1-hex Ice',
    terrain: HexTerrain.ice,
    landPrefix: PiecePrefixes.ice,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.ice3]: {
    id: Pieces.ice3,
    title: '3-hex Ice',
    terrain: HexTerrain.ice,
    landPrefix: PiecePrefixes.ice,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 0,
  },
  [Pieces.ice4]: {
    id: Pieces.ice4,
    title: '4-hex Ice',
    terrain: HexTerrain.ice,
    landPrefix: PiecePrefixes.ice,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 4,
    template: '4',
    height: 0,
  },
  [Pieces.ice6]: {
    id: Pieces.ice6,
    title: '6-hex Ice',
    terrain: HexTerrain.ice,
    landPrefix: PiecePrefixes.ice,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 6,
    template: '6',
    height: 0,
  },
  [Pieces.shadow1]: {
    id: Pieces.shadow1,
    title: '1-hex Shadow',
    terrain: HexTerrain.shadow,
    landPrefix: PiecePrefixes.shadow,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 1,
    template: '1',
    height: 0,
  },
  [Pieces.shadow3]: {
    id: Pieces.shadow3,
    title: '3-hex Shadow',
    terrain: HexTerrain.shadow,
    landPrefix: PiecePrefixes.shadow,
    isHexTerrainPiece: true,
    isObstaclePiece: false,
    size: 3,
    template: '3',
    height: 0,
  },
  // LAUR WALL
  [Pieces.laurWallSquarePillar]: {
    id: Pieces.laurWallSquarePillar,
    title: 'Laur Pillar',
    terrain: HexTerrain.laurWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 12, // game height was 10, errata 9
  },
  [Pieces.laurWallPillarStackable]: {
    id: Pieces.laurWallPillarStackable,
    title: 'Laur Pillar (Stackable)',
    terrain: HexTerrain.laurWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 12, // game height was 10, errata 9
  },
  [Pieces.laurWallTrianglePillar]: {
    id: Pieces.laurWallTrianglePillar,
    title: 'Laur Triangle Pillar',
    terrain: HexTerrain.laurWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 12, // game height was 10, errata 9
  },
  [Pieces.laurWallShort]: {
    id: Pieces.laurWallShort,
    title: 'Laur Wall (Short)',
    terrain: HexTerrain.laurWallAddon,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 1,
    template: Pieces.laurWallShort,
    height: 12, // game height is 10
  },
  [Pieces.laurWallShortStackable]: {
    id: Pieces.laurWallShortStackable,
    title: 'Laur Wall (Short) (Stackable)',
    terrain: HexTerrain.laurWallAddon,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 1,
    template: Pieces.laurWallShort,
    height: 12, // game height is 10
  },
  [Pieces.laurWallLong]: {
    id: Pieces.laurWallLong,
    title: 'Laur Wall (Long)',
    terrain: HexTerrain.laurWallAddon,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 1,
    template: Pieces.laurWallLong,
    height: 12, // game height is 10
  },
  [Pieces.laurWallLongStackable]: {
    id: Pieces.laurWallLongStackable,
    title: 'Laur Wall (Long) (Stackable)',
    terrain: HexTerrain.laurWallAddon,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 1,
    template: Pieces.laurWallLongStackable,
    height: 12, // game height is 10
  },
  [Pieces.laurWallArch]: {
    id: Pieces.laurWallArch,
    title: 'Laur Wall Arch',
    terrain: HexTerrain.laurWallAddon,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 1,
    template: Pieces.laurWallArch,
    height: 12, // game height is 10
  },
  [Pieces.laurWallRuin1]: {
    id: Pieces.laurWallRuin1,
    title: 'Laur Wall Ruin',
    terrain: HexTerrain.laurWallAddon,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 1,
    template: Pieces.laurWallRuin1,
    height: 10, //smallest ruin, no top part, game height is undefined (doesn't block adjacency)
  },
  [Pieces.laurWallRuin2]: {
    id: Pieces.laurWallRuin2,
    title: 'Laur Wall Ruin (style 2)',
    terrain: HexTerrain.laurWallAddon,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 1,
    template: Pieces.laurWallRuin2,
    height: 12, // game height is undefined (doesn't block adjacency)
  },
  [Pieces.laurWallRuin3]: {
    id: Pieces.laurWallRuin3,
    title: 'Laur Wall Ruin (style 3)',
    terrain: HexTerrain.laurWallAddon,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 1,
    template: Pieces.laurWallRuin3,
    height: 12, // game height is undefined (doesn't block adjacency)
  },
  // RUINS
  [Pieces.ruins2]: {
    id: Pieces.ruins2,
    title: 'Ruins (2)',
    terrain: HexTerrain.ruin,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 8,
    template: Pieces.ruins2,
    height: 5, // these have a template done, but pending vertex data approach
  },
  [Pieces.ruins3]: {
    id: Pieces.ruins3,
    title: 'Ruins (3)',
    terrain: HexTerrain.ruin,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 10,
    template: Pieces.ruins3,
    height: 5, // these have a template done, but pending vertex data approach
  },
  [Pieces.fortifiedWall]: {
    id: Pieces.fortifiedWall,
    title: 'Fortified Wall',
    terrain: HexTerrain.fortifiedWall,
    isHexTerrainPiece: false,
    isObstaclePiece: false,
    size: 8,
    template: Pieces.fortifiedWall,
    height: 10,
  },
  [Pieces.marvel]: {
    id: Pieces.marvel,
    title: 'Warehouse Ruin',
    terrain: HexTerrain.marvelRuin,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 6,
    template: Pieces.marvel,
    height: 9, // rulebook height is warehoue upper floor 9 levels above base, the windows need a vertical obstruction template done + reality testing
  },
  [Pieces.marvelBroken]: {
    id: Pieces.marvelBroken,
    title: 'Warehouse Ruin - No Wall',
    terrain: HexTerrain.marvelRuin,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 6,
    template: Pieces.marvel,
    height: 9, // rulebook height is warehoue upper floor 9 levels above base, the windows need a vertical obstruction template done + reality testing
  },
  [Pieces.marvelNoUpper]: {
    id: Pieces.marvelNoUpper,
    title: 'Warehouse Ruin - No Upper Floor',
    terrain: HexTerrain.marvelRuin,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 6,
    template: Pieces.marvel,
    height: 9, // rulebook height is warehoue upper floor 9 levels above base, the windows need a vertical obstruction template done + reality testing
  },
  [Pieces.marvelNoUpperBroken]: {
    id: Pieces.marvelNoUpperBroken,
    title: 'Warehouse Ruin- No Upper Floor / No Wall',
    terrain: HexTerrain.marvelRuin,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 6,
    template: Pieces.marvel,
    height: 9, // rulebook height is warehoue upper floor 9 levels above base, the windows need a vertical obstruction template done + reality testing
  },
  // HEX OBSTACLES
  [Pieces.snowTree10]: {
    id: Pieces.snowTree10,
    title: 'Snow Covered Evergreen Tree (10)',
    terrain: HexTerrain.snowTree,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 10,
  },
  [Pieces.snowTree12]: {
    id: Pieces.snowTree12,
    title: 'Snow Covered Evergreen Tree (12)',
    terrain: HexTerrain.snowTree,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 12,
  },
  [Pieces.tree10]: {
    id: Pieces.tree10,
    title: 'Evergreen Tree (10)',
    terrain: HexTerrain.tree,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 10,
  },
  [Pieces.tree11]: {
    id: Pieces.tree11,
    title: 'Evergreen Tree (11)',
    terrain: HexTerrain.tree,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 11,
  },
  [Pieces.tree12]: {
    id: Pieces.tree12,
    title: 'Evergreen Tree (12)',
    terrain: HexTerrain.tree,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 12,
  },
  [Pieces.tree415]: {
    id: Pieces.tree415,
    title: 'Large Evergreen Tree (15)',
    terrain: HexTerrain.tree,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 4,
    template: '4',
    height: 15,
  },
  [Pieces.swampBrush10]: {
    id: Pieces.swampBrush10,
    title: 'Swamp Underbrush',
    terrain: HexTerrain.brush,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 10,
  },
  [Pieces.laurBrush10]: {
    id: Pieces.laurBrush10,
    title: 'Laur Underbrush ',
    terrain: HexTerrain.brush,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 10,
  },
  [Pieces.laurPalm13]: {
    id: Pieces.laurPalm13,
    title: 'Laur Jungle Tree (13)',
    terrain: HexTerrain.palm,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 13,
  },
  [Pieces.laurPalm14]: {
    id: Pieces.laurPalm14,
    title: 'Laur Jungle Tree (14)',
    terrain: HexTerrain.palm,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 14,
  },
  [Pieces.laurPalm15]: {
    id: Pieces.laurPalm15,
    title: 'Laur Jungle Tree (15)',
    terrain: HexTerrain.palm,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 15,
  },
  [Pieces.brush9]: {
    id: Pieces.brush9,
    title: 'Ticalla Underbrush',
    terrain: HexTerrain.brush,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 9,
  },
  [Pieces.palm14]: {
    id: Pieces.palm14,
    title: 'Ticalla Jungle Palm (14)',
    terrain: HexTerrain.palm,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 14,
  },
  [Pieces.palm15]: {
    id: Pieces.palm15,
    title: 'Ticalla Jungle Palm (15)',
    terrain: HexTerrain.palm,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 15,
  },
  [Pieces.palm16]: {
    id: Pieces.palm16,
    title: 'Ticalla Jungle Palm (16)',
    terrain: HexTerrain.palm,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 16,
  },
  [Pieces.shroudshroom7]: {
    id: Pieces.shroudshroom7,
    title: 'Shroudshroom (7)',
    terrain: HexTerrain.shroudshroom,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 7,
  },
  [Pieces.shroudshroom10]: {
    id: Pieces.shroudshroom10,
    title: 'Shroudshroom (10)',
    terrain: HexTerrain.shroudshroom,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 10,
  },
  [Pieces.shroudshroom13]: {
    id: Pieces.shroudshroom13,
    title: 'Shroudshroom (13)',
    terrain: HexTerrain.shroudshroom,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 3,
    template: '3',
    height: 13,
  },
  [Pieces.shipWall]: {
    id: Pieces.shipWall,
    title: 'Ship Wall',
    terrain: HexTerrain.shipWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 3,
    template: Pieces.shipWall,
    height: 10,
  },
  [Pieces.shipBow]: {
    id: Pieces.shipBow,
    title: 'Ship Bow',
    terrain: HexTerrain.shipBow,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 5,
    template: Pieces.shipBow,
    height: 13,
  },
  [Pieces.cannon]: {
    id: Pieces.cannon,
    title: 'Cannon',
    terrain: HexTerrain.cannon,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 3,
  },
  [Pieces.ropeLadder]: {
    id: Pieces.ropeLadder,
    title: 'Rope Ladder',
    terrain: HexTerrain.ropeLadder,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 7,
  },
  // HEX-OBSTACLES with BASE fluid tiles
  [Pieces.glacier1]: {
    id: Pieces.glacier1,
    title: 'Glacier (1)',
    terrain: HexTerrain.glacier,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 7, // eventhough it definitely spans 8 hex-heights in reality!
  },
  [Pieces.glacier3]: {
    id: Pieces.glacier3,
    title: 'Glacier (3)',
    terrain: HexTerrain.glacier,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 3,
    template: '3',
    height: 9, // untested in reality (heights clockwise from origin: 5,9,7)
  },
  [Pieces.glacier4]: {
    id: Pieces.glacier4,
    title: 'Glacier (4)',
    terrain: HexTerrain.glacier,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 4,
    template: '4',
    height: 8, // untested in reality (heights clockwise from origin: 7,11,9,11)
  },
  [Pieces.glacier6]: {
    id: Pieces.glacier6,
    title: 'Glacier (6)',
    terrain: HexTerrain.glacier,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 6,
    template: '6',
    height: 17, // untested in reality (heights clockwise from origin: 17,17,9,17,17,9)
  },
  [Pieces.outcrop1]: {
    id: Pieces.outcrop1,
    title: 'Rock Outcrop (1)',
    terrain: HexTerrain.outcrop,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 7, // eventhough it definitely spans 8 hex-heights in reality!
  },
  [Pieces.outcrop3]: {
    id: Pieces.outcrop3,
    title: 'Rock Outcrop (3)',
    terrain: HexTerrain.outcrop,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 3,
    template: '3',
    height: 9, // untested in reality
  },
  [Pieces.lavaRockOutcrop1]: {
    id: Pieces.lavaRockOutcrop1,
    title: 'Lava Rock Outcrop (1)',
    terrain: HexTerrain.lavaRockOutcrop,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 7, // eventhough it definitely spans 8 hex-heights in reality!
  },
  [Pieces.lavaRockOutcrop3]: {
    id: Pieces.lavaRockOutcrop3,
    title: 'Lava Rock Outcrop (3)',
    terrain: HexTerrain.lavaRockOutcrop,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 3,
    template: '3',
    height: 9, // untested in reality
  },
  [Pieces.hive]: {
    id: Pieces.hive,
    title: 'Marro Hive',
    terrain: HexTerrain.hive,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 6,
    template: '6',
    height: 17, // untested in reality
  },
  // CASTLE
  [Pieces.castleBaseEnd]: {
    id: Pieces.castleBaseEnd,
    title: 'Fortress Wall Base (End)',
    terrain: HexTerrain.castleBase,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.castleBaseStraight]: {
    id: Pieces.castleBaseStraight,
    title: 'Fortress Wall Base (Straight)',
    terrain: HexTerrain.castleBase,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.castleBaseCorner]: {
    id: Pieces.castleBaseCorner,
    title: 'Fortress Wall Base (Corner)',
    terrain: HexTerrain.castleBase,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 1,
  },
  [Pieces.castleArch]: {
    id: Pieces.castleArch,
    title: 'Fortress Doorway',
    terrain: HexTerrain.castleWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: Pieces.castleArch,
    height: 9,
  },
  [Pieces.castleArchNoDoor]: {
    id: Pieces.castleArchNoDoor,
    title: 'Fortress Doorway (No door)',
    terrain: HexTerrain.castleWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: Pieces.castleArch,
    height: 9,
    isUninventoried: true,
  },
  [Pieces.castleWallEnd]: {
    id: Pieces.castleWallEnd,
    title: 'Fortress Wall (End)',
    terrain: HexTerrain.castleWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 9,
  },
  [Pieces.castleWallStraight]: {
    id: Pieces.castleWallStraight,
    title: 'Fortress Wall (Straight)',
    terrain: HexTerrain.castleWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 9,
  },
  [Pieces.castleWallCorner]: {
    id: Pieces.castleWallCorner,
    title: 'Fortress Wall (Corner)',
    terrain: HexTerrain.castleWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: '1',
    height: 9,
  },
  [Pieces.ladder]: {
    id: Pieces.ladder,
    title: 'Ladder',
    terrain: HexTerrain.ladder,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: Pieces.ladder,
    height: 2,
  },
  [Pieces.battlement]: {
    id: Pieces.battlement,
    title: 'Battlement',
    terrain: HexTerrain.battlement,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 1,
    template: Pieces.battlement,
    height: 5, // see example 8, pg 3 of Fortress rulebook https://github.com/Dissolutio/heroscape-map-editor/issues/1
  },
  [Pieces.roadWall]: {
    id: Pieces.roadWall,
    title: 'Stone Wall',
    terrain: HexTerrain.roadWall,
    isHexTerrainPiece: false,
    isObstaclePiece: true,
    size: 4,
    template: Pieces.roadWall,
    height: 3, // see example 3-6, pg 3 of RoadToTheForgottenForest rulebook https://github.com/Dissolutio/heroscape-map-editor/issues/2
  },
}

// Dynamically add specific glyph inventory entries for all defined glyphs.
// Inventory key will be PiecePrefixes.glyph + glyph.id (e.g. 'yattack').
;(function addGlyphs() {
  const allGlyphArrays = [
    powerGlyphs,
    treasureGlyphs,
    marvelGlyphs,
    c3vGlyphs,
    c3vPlaytestGlyphs,
    customGlyphs,
  ]
  for (const glyphArr of allGlyphArrays) {
    for (const g of glyphArr) {
      try {
        const key = `${PiecePrefixes.glyph}${g.id}`
        // Avoid overwriting existing entries
        if (piecesSoFar[key]) continue
        // objective type glyphs will be
        const terrain = g.terrain
        piecesSoFar[key] = {
          id: key,
          title: g.name ?? g.shortName ?? g.id,
          terrain,
          isHexTerrainPiece: false,
          isObstaclePiece: true,
          isOverlayPiece: true,
          size: 1,
          template: '1',
          height: 0,
          glyphLetter: g.glyphLetter,
        }
      } catch {
        // ignore errors here
      }
    }
  }
})()
